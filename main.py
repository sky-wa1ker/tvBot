...
#Code - 0.75
#Last Change - hid sensitive info.
...
async def movie(ctx, *, movie_search_str):
    encoded_search_str = urllib.parse.quote(movie_search_str)
    ...
    movie_result_dict = response.json()
...
async def person(ctx, *, actor_search_str):
    ...
    if (actor_result_dict.get('total_results')) == 0 or ((actor_result_dict.get('results')[0])['known_for_department']) != 'Acting':
        await ctx.send('Your search returned 0 results.')

    else:
        actor_id = ((actor_result_dict.get('results')[0])['id'])
        id_query_response = requests.get(f'https://api.themoviedb.org/3/person/{actor_id}?api_key={api_key}&language=en-US')
...
@client.command(aliases = ['popularmovies'])
async def popularmovie(ctx):
